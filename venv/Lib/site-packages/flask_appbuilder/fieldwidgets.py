...
class Select2SubordinateAJAXWidget(object):
    data_template = ('<input class="input-group my_select2_ajax_subordinate" %(text)s"></input>')

    def __init__(self, main_id, endpoint, extra_classes=None, style=None):
        self.endpoint = endpoint
    ...
    def __call__(self, field, **kwargs):
        ...
        kwargs.setdefault('name', field.name)
        kwargs.setdefault('endpoint', self.endpoint)
        kwargs.setdefault('main_id', self.main_id)
        kwargs.setdefault('style', self.style)
        input_classes = 'input-group my_select2_ajax'
        if self.extra_classes:
            input_classes = input_classes + ' ' + self.extra_classes
