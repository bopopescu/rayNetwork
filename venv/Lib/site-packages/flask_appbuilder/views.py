...
from .widgets import GroupFormListWidget, ListMainWidget
...
class MainDetailView(BaseCRUDView):
    """
        Implements behaviour for controlling two CRUD views
        linked by PK and FK, in a main/detail type with
        two lists.

        Main view will behave like a left menu::

            class DetailView(ModelView):
                datamodel = SQLAInterface(DetailTable, db.session)

            class MainView(MainDetailView):
                datamodel = SQLAInterface(MainTable, db.session)
                related_views = [DetailView]

    """

    list_template = 'appbuilder/general/model/left_main_detail.html'
    list_widget = ListMainWidget
    main_div_width = 2
    """
        Set to configure bootstrap class for main grid size
    """
    ...
    def list(self, pk=None):
        ...
        return self.render_template(self.list_template,
                                    ...
                                    main_div_width=self.main_div_width)
