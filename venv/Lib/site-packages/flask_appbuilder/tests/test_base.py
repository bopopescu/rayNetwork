...
from flask_appbuilder.views import MainDetailView, CompactCRUDMixin
...
class FlaskTestCase(unittest.TestCase):
    def setUp(self):
        ...
        class Model1MainView(MainDetailView):
            datamodel = SQLAInterface(Model1)
            related_views = [Model2View]
        ...
        class Model1MainChartView(MainDetailView):
            datamodel = SQLAInterface(Model1)
        ...
        self.appbuilder.add_view(Model1MainView, "Model1Main", category='Model1')
        self.appbuilder.add_view(Model1MainChartView, "Model1MainChart", category='Model1')
    ...
    def test_main_detail_view(self):
        """
            Test Main detail view
        """
        client = self.app.test_client()
        self.login(client, DEFAULT_ADMIN_USER, DEFAULT_ADMIN_PASSWORD)
        self.insert_data2()
        rv = client.get('/model1mainview/list/')
        eq_(rv.status_code, 200)
        rv = client.get('/model1mainview/list/1')
        eq_(rv.status_code, 200)

        rv = client.get('/model1mainchartview/list/')
        eq_(rv.status_code, 200)
        rv = client.get('/model1mainchartview/list/1')
        eq_(rv.status_code, 200)
