...
class BendersAlgorithm(PySPConfiguredObject):
    ...
    def _declare_options(cls, options=None):
        ...
        safe_declare_unique_option(
            ...
            PySPConfigValue(
                ...
                description=(
                    ...
                    "main benders problem each iteration. "
        ...
        safe_declare_unique_option(
            options,
            "main_disable_warmstart",
            PySPConfigValue(
                False,
                domain=bool,
                description=(
                    "Disable warm-start of the benders main "
        ...
        safe_declare_unique_option(
            ...
            PySPConfigValue(
                ...
                description=(
                    ...
                    "the main benders problem. Default is cplex."
                ),
                doc=None,
                visibility=0),
            ap_group=_benders_group_label)
        safe_declare_unique_option(
            ...
            PySPConfigValue(
                ...
                domain=_domain_must_be_str,
                description=(
                    "The type of IO used to execute the main "
        ...
        safe_declare_unique_option(
            ...
            PySPConfigValue(
                ...
                description=(
                    "Specifies the mipgap for the main benders solves."
                ),
                doc=None,
                visibility=0),
            ap_group=_benders_group_label)
        safe_declare_unique_option(
            ...
            PySPConfigValue(
                ...
                description=(
                    "Persistent solver options used when solving the main "
        ...
        safe_declare_unique_option(
            options,
            "main_output_solver_log",
            PySPConfigValue(
                False,
                domain=bool,
                description=(
                    "Output solver log during solves of the main problem."
                ),
                doc=None,
                visibility=0),
            ap_group=_benders_group_label)
        safe_declare_unique_option(
            options,
            "main_keep_solver_files",
            PySPConfigValue(
                False,
                domain=bool,
                description=(
                    "Retain temporary input and output files for main "
                    "benders solves."
                ),
                doc=None,
                visibility=0),
    ...
    def __init__(self, manager, *args, **kwds):
        ...
        ############################################
        # history of main bounds
        self.main_bound_history = OrderedDict()
        ...
        # The following attributes will be modified by the
        # build_main_problem() method. They will be
        ...
        self._main_solver = None
        # setup the main solver
        self._main_solver = SolverFactory(
            self.get_option("main_solver"),
            solver_io=self.get_option("main_solver_io"))
        if isinstance(self._main_solver, PersistentSolver):
            raise TypeError("BendersAlgorithm does not yet support "
                            "PersistentSolver types for the main problem")
        if len(self.get_option("main_solver_options")):
            if type(self.get_option("main_solver_options")) is tuple:
                self._main_solver.set_options(
                    "".join(self.get_option("main_solver_options")))
            else:
                self._main_solver.set_options(
                    self.get_option("main_solver_options"))
        if self.get_option("main_mipgap") is not None:
            self._main_solver.options.mipgap = \
                self.get_option("main_mipgap")
    ...
    # constructing a main problem.
    #

    def build_main_problem(self, **kwds):
        """
        Build a main problem to add cuts to. This method
        ...
        to include on the main that were specified on the options
        object used to initialize this class.
        """

        # check whether to override the
        # "main_include_scenarios" option
        if "include_scenarios" not in kwds:
            include_scenarios = self.get_option("main_include_scenarios")
        else:
        ...
        if (include_scenarios is None) or \
           (len(include_scenarios) == 0):
            main_scenario_tree = scenario_tree.make_compressed(
                [scenario_tree.scenarios[0].name],
                normalize=False)
        else:
            print("Number of scenarios included in Benders main problem: %s"
                  % (len(include_scenarios)))
            main_scenario_tree = scenario_tree.make_compressed(
                include_scenarios,
                normalize=False)

        main_rootnode = main_scenario_tree.findRootNode()
        main_firststage = main_scenario_tree.stages[0]
        assert main_firststage is main_rootnode.stage
        main_secondstage = main_scenario_tree.stages[1]
        assert len(main_scenario_tree.stages) == 2

        main_scenario_instances = \
            scenario_tree._scenario_instance_factory.\
            construct_instances_for_scenario_tree(main_scenario_tree)
        # with the scenario instances now available, link the
        # referenced objects directly into the scenario tree.
        main_scenario_tree.linkInInstances(main_scenario_instances,
                                             create_variable_ids=True)

        main = create_ef_instance(main_scenario_tree)
        if (include_scenarios is None) or \
           (len(include_scenarios) == 0):
            main._scenarios_included = set()
            remove_ssc = True
        else:
            main._scenarios_included = \
        ...
        num_first_stage_constraints = len(list(itertools.chain(
            main.component_data_objects(SOSConstraint,
                                          active=True,
                                          descend_into=False),
            main.component_data_objects(Constraint,
                                          active=True,
                                          descend_into=False))))
        # now count the first-stage constraints on the scenario
        # instances included in the main ef
        for scenario in main_scenario_tree.scenarios:
            instance = scenario._instance
            for constraint_data in itertools.chain(
                ...
                if node.stage is not main_firststage:
                    assert node.stage is main_secondstage
                    if remove_ssc:
        ...
        find_active_objective(main, safety_checks=True).deactivate()
        ...
        for scenario in scenario_tree.scenarios:
            if scenario.name not in main._scenarios_included:
        ...
        assert not hasattr(main, alpha_varname)
        main.add_component(alpha_varname,Var())
        main_alpha = main.find_component(alpha_varname)

        alpha_bundles_index_name = "PYSP_BENDERS_BUNDLE_ALPHA_SSC_INDEX"
        assert not hasattr(main, alpha_bundles_index_name)
        main.add_component(alpha_bundles_index_name,
                             RangeSet(0,len(nonempty_cut_bundles)-1))
        bundle_alpha_index = main.find_component(alpha_bundles_index_name)

        alpha_bundles_name = "PYSP_BENDERS_BUNDLE_ALPHA_SSC"
        assert not hasattr(main, alpha_bundles_name)
        main.add_component(alpha_bundles_name,
                             Var(bundle_alpha_index))
        bundle_alpha = main.find_component(alpha_bundles_name)

        cut_bundles_list_name = "PYSP_BENDERS_CUT_BUNDLES_SSC"
        assert not hasattr(main, cut_bundles_list_name)
        setattr(main, cut_bundles_list_name, nonempty_cut_bundles)
        alpha_cut_constraint_name = "PYSP_BUNDLE_AVERAGE_ALPHA_CUT_SSC"
        assert not hasattr(main, alpha_cut_constraint_name)
        if objective_sense == minimize:
            main.add_component(
                alpha_cut_constraint_name,
                Constraint(expr=main_alpha >= sum(bundle_alpha[i]
        ...
        if (include_scenarios is None) or \
           (len(include_scenarios) == 0):
            assert len(main_scenario_tree.scenarios) == 1
            main_cost_expr = main.find_component(
                main_scenario_tree.scenarios[0].name).find_component(
                    main_rootnode._cost_variable[0])\
                    [main_rootnode._cost_variable[1]]
        else:
            # NOTE: We include the first-stage cost expression for
            #       each of the scenarios included in the main with
            ...
            for scenario in main_scenario_tree.scenarios:
                ...
                assert scenario_rootnode is main_rootnode
                ...
                main_cost_expr += scenario.probability * \
                                    (rootnode_cost_expr / normalization + \
                                     leafnode_cost_expr)
        benders_objective_name = "PYSP_BENDERS_OBJECTIVE"
        assert not hasattr(main, benders_objective_name)
        main.add_component(
            benders_objective_name,
            Objective(expr=main_cost_expr + main_alpha,
                      sense=objective_sense))
        main_objective = main.find_component(
            benders_objective_name)
        cutlist_constraint_name = "PYSP_BENDERS_CUTS_SSC"
        assert not hasattr(main, cutlist_constraint_name)
        ...
        main.add_component(cutlist_constraint_name,
                             XConstraintList())
    ...
    def add_cut(self, benders_cut, ignore_cut_bundles=False):
        ...
        main problem. The optional keyword ignore_cut_bundles
        can be used generate the cut using the single main
        alpha cut variable rather than over the possibly many
        bundle cut groups.
        """

        if self.main is None:
            raise RuntimeError("The main problem has not been constructed."
                               "Call the build_main_problem() method to "
                               "construct it.")
        ...
        main = self.main
        rootnode = scenario_tree.findRootNode()
        main_variable = main.find_component(
            "MASTER_BLEND_VAR_"+str(rootnode.name))
        benders_cuts = main.find_component(
            "PYSP_BENDERS_CUTS_SSC")
        main_alpha = main.find_component(
            "PYSP_BENDERS_ALPHA_SSC")
        bundle_alpha = main.find_component(
        ...
        if not ignore_cut_bundles:
            for i, cut_scenarios in enumerate(
                    getattr(main, "PYSP_BENDERS_CUT_BUNDLES_SSC")):

                for scenario_name in cut_scenarios:
                    assert scenario_name not in main._scenarios_included
                    ...
                    cut_expression += \
                        ...
                        (scenario_ssc + \
                         sum(scenario_duals[variable_id] * \
                             (main_variable[variable_id] - xhat[variable_id]) \
            ...
            for scenario in scenario_tree.scenarios:
                if scenario.name in main._scenarios_included:
                    continue
    ...
    def extract_main_xhat(self):
        ...
        main_variable = main.find_component(
            "MASTER_BLEND_VAR_"+str(rootnode.name))
        return dict((variable_id, main_variable[variable_id].value)
                    for variable_id in rootnode._standard_variable_ids
                    if not main_variable[variable_id].stale)
    ...
    def solve_main(self):
        ...
        common_kwds = {
            ...
            'tee':self.get_option("main_output_solver_log"),
            'keepfiles':self.get_option("main_keep_solver_files"),
            'symbolic_solver_labels':self.get_option("main_symbolic_solver_labels")}

        if (not self.get_option("main_disable_warmstart")) and \
           (self._main_solver.warm_start_capable()):
            results = self._main_solver.solve(self.main,
                                                warmstart=True,
                                                **common_kwds)
        else:
            results = self._main_solver.solve(self.main,
...
#        print_dictionary(problem_statistics)
        ...
        if output_solver_log:
            ...
            print("%6s %16s %16s %11s  %30s"
                  % ("Iter","Main Bound","Best Incumbent","Gap","Solution Times [s]"))
            print("%6s %16s %16s %11s  %10s %10s %10s %10s"
                  % ("","","","","Main","Sub Min","Sub Max","Cumm"))
            print("-"*width_log_table)

        main_alpha = main.find_component(
            "PYSP_BENDERS_ALPHA_SSC")
        main_bundles_alpha = main.find_component(
            "PYSP_BENDERS_BUNDLE_ALPHA_SSC")
        main_objective = main.find_component(
        ...
        for i in xrange(1, max_iterations + 1):
            ...
            if (i == 1) and (len(self.cut_pool) == 0):
                ...
                # use the main objective as a lower bound
                main_alpha.fix(0.0)
                main_bundles_alpha.fix(0.0)

            start_time_main = time.time()
            results_main = self.solve_main()
            if len(results_main.solution) == 0:
                raise RuntimeError("Solve failed for main; no solutions generated")
            if results_main.solver.termination_condition != \
               TerminationCondition.optimal:
                logger.warning(
                    "Main solve did not generate an optimal solution:\n"
                    "Solver Status: %s\n"
                    "Solver Termination Condition: %s\n"
                    "Solution Status: %s\n"
                    % (str(results_main.solver.status),
                       str(results_main.solver.termination_condition),
                       str(results_main.solution(0).status)))
            main.solutions.load_from(results_main)
            stop_time_main = time.time()

            if main_alpha.fixed:
                assert i == 1
                assert main_alpha.value == 0.0
                current_main_bound = \
                    float('-inf') if (objective_sense is minimize) else float('inf')
                main_alpha.free()
                main_bundles_alpha.free()
            else:
                current_main_bound = value(main_objective)
                # account for any optimality gap
                solution0 = results_main.solution(0)
                if hasattr(solution0, "gap") and \
                   (solution0.gap is not None) and \
                   (solution0.gap is not undefined):
                    if objective_sense == minimize:
                        current_main_bound -= solution0.gap
                    else:
                        current_main_bound += solution0.gap
                else:
            ...
            self.optimality_gap = \
                abs(best_main_bound - self.incumbent_objective) / \
            ...
            if output_solver_log:
                ...
                print("%6d %16.4f %16.4f %11.3f%% %10.2f %10.2f %10.2f %10.2f"
                      % (i, current_main_bound, self.incumbent_objective,
                         self.optimality_gap*100, stop_time_main - start_time_main,
            ...
            # loop. Otherwise, add the new cut to the main
        ...
        if len(self.main_bound_history):
            if objective_sense is minimize:
                self.bound = max(self.main_bound_history.values())
            else:
                self.bound = min(self.main_bound_history.values())
