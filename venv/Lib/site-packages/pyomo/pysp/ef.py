...
def create_ef_instance(scenario_tree,
    ...
    # constraints/objectives defined in a subsequent pass. main
    ...
    if verbose_output:
        print("Creating variables for main binding instance")
    ...
    for stage in scenario_tree._stages[:-1]: # skip the leaf stage
        ...
        for tree_node in stage._tree_nodes:
            ...
            # create the main blending variable and constraints for this node
            main_blend_variable_name = \
                "MASTER_BLEND_VAR_"+str(tree_node._name)
            main_blend_constraint_name = \
                "MASTER_BLEND_CONSTRAINT_"+str(tree_node._name)

            # don't create main variables for derived
            ...
            # scenario variables
            main_variable_index = Set(initialize=sorted(tree_node._standard_variable_ids),
                                        ordered=True,
                                        name=main_blend_variable_name+"_index")

            binding_instance.add_component(main_blend_variable_name+"_index",
                                           main_variable_index)

            main_variable = Var(main_variable_index,
                                  name=main_blend_variable_name)

            binding_instance.add_component(main_blend_variable_name,
                                           main_variable)

            main_constraint = ConstraintList(name=main_blend_constraint_name)

            binding_instance.add_component(main_blend_constraint_name,
                                           main_constraint)

            tree_node_variable_datas = tree_node._variable_datas
            for variable_id in sorted(tree_node._standard_variable_ids):
                main_vardata = main_variable[variable_id]
                ...
                if not tree_node.is_variable_fixed(variable_id):
                    for scenario_vardata, scenario_probability in vardatas:
                        _cmap[scenario_vardata] = main_constraint.add(
                            (main_vardata-scenario_vardata,0.0))
    ...
    if generate_weighted_cvar:
        ...
        # add the constraint to compute the main CVaR variable value. iterate
...
def solve_ef(main_instance, options):
        ...
        with SolverManagerFactory(options.solver_manager_type) as ef_solver_manager:
            ...
            if (results.solver.status == SolverStatus.ok) and \
               ((results.solver.termination_condition == TerminationCondition.optimal) or \
                ...
                main_instance.solutions.load_from(results)
                return results
