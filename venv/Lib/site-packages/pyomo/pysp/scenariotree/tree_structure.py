...
class ScenarioTreeNode(object):
    ...
    def __init__(self, name, conditional_probability, stage):
        ...
        # for when pyomo instances no longer live on the main node
        # when using PHPyro
        self._integer = set()
        self._binary = set()
    ...
    def linkInInstances(self,
        ...
        for scenario_name in scenario_names:
            ...
            if self.contains_scenario(scenario_name):
                main_has_instance[scenario_name] = False
                if main_scenario_tree is not None:
                    main_scenario = \
                        main_scenario_tree.get_scenario(scenario_name)
                    if main_scenario._instance is not None:
                        main_has_instance[scenario_name] = True
        ...
        self.populateVariableIndicesAndValues(
            ...
            main_scenario_tree=main_scenario_tree,
        ...
        for scenario_name in scenario_names:
            ...
            if self.contains_scenario(scenario_name):
                ...
                if main_has_instance[scenario_name]:
                    main_scenario = \
                        main_scenario_tree.get_scenario(scenario_name)
                    scenario._instance_cost_expression = \
                        main_scenario._instance_cost_expression
                    scenario._instance_objective = \
                        main_scenario._instance_objective
                    scenario._objective_sense =\
                        main_scenario._objective_sense
                    scenario._objective_name = main_scenario
                    continue
    ...
    def populateVariableIndicesAndValues(self,
        ...
        if (create_variable_ids == True) and \
           (main_scenario_tree != None):
            raise RuntimeError(
                ...
                "create_variable_ids=True and main_scenario_tree!=None")
        ...
        for stage in self._stages:
            ...
            for tree_node in tree_node_list:
                name_index_to_id_map = None
                if main_scenario_tree is not None:
                    name_index_to_id_map = main_scenario_tree.\
