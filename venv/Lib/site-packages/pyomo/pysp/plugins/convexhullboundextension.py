...
class convexhullboundextension(pyomo.common.plugin.SingletonPlugin, _PHBoundBase):
    ...
    def _construct_bundle_dual_main_model(self, ph):

        self._main_model = ConcreteModel()
        ...
        self._main_model.TheObjective = Objective(sense=minimize, rule=obj_rule)

        self._main_model.W_Balance = ConstraintList()
            ...
            for tree_node in stage._tree_nodes:
                ...
                for idx in tree_node._standard_variable_ids:
                    ...
                    for scenario in tree_node._scenarios:
                        ...
                        new_w_variable_name = "WVAR_"+str(tree_node._name)+"_"+str(scenario._name)
                        w_variable = self._main_model.find_component(new_w_variable_name)
                        expr += scenario_probability * w_variable[idx]

                    self._main_model.W_Balance.add(expr == 0.0)

        # we can't populate until we see data from PH....
        self._main_model.V_Bound = ConstraintList()

#        self._main_model.pprint()

    #
    # populate the main bundle model from the PH parameters
    #
    def _populate_bundle_dual_main_model(self, ph):
...
#        print "PAST VAR VALUES=",self._past_var_values
        ...
        for scenario in ph._scenario_tree._scenarios:
            for tree_node in scenario._node_list[:-1]:
                ...
                w_k_parameter = \
                    self._main_model.find_component(new_w_k_parameter_name)
        ...
        for scenario in ph._scenario_tree._scenarios:
            ...
            v_var = getattr(self._main_model, "V_"+str(scenario_name))
            ...
            self._main_model.V_Bound.add(v_var <= expr)

#        print "V_BOUNDS CONSTRAINT:"
#        self._main_model.V_Bound.pprint()
        with SolverFactory(ph._solver_type,
            ...
            if isinstance(solver, PersistentSolver):
                solver.set_instance(self._main_model)
            results = solver.solve(self._main_model)
            self._main_model.solutions.load_from(results)
...
#        self._main_model.pprint()
    ...
    #
    # take the weights from the current convex hull main problem
    ...
    def post_ph_initialization(self, ph):
        ...
        self._construct_bundle_dual_main_model(ph)
    ...
    def post_iteration_0_solves(self, ph):
        ...
        self._populate_bundle_dual_main_model(ph)
