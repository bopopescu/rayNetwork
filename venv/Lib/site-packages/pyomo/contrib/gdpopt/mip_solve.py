...
from pyomo.contrib.gdpopt.data_class import MainProblemResult
...
def solve_linear_GDP(linear_GDP_model, solve_data, config):
    ...
    config.call_before_main_solve(m, solve_data)
...
def solve_LOA_main(solve_data, config):
    """Solve the augmented lagrangean outer approximation main problem."""
    ...
    if mip_result.feasible:
        ...
        if solve_data.main_iteration == 1:
            config.logger.warning(
