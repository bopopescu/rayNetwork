...
from pyomo.contrib.gdpopt.cut_generation import (add_integer_cut,
                                                 ...
                                                 add_affine_cuts)
from pyomo.contrib.gdpopt.mip_solve import solve_LOA_main
...
def GDPopt_iteration_loop(solve_data, config):
    ...
    """
    while solve_data.main_iteration < config.iterlim:
        # Set iteration counters for new main iteration.
        solve_data.main_iteration += 1
        solve_data.mip_iteration = 0
        solve_data.nlp_iteration = 0

        # print line for visual display
        config.logger.info(
            '---GDPopt Main Iteration %s---'
            % solve_data.main_iteration)

        # solve linear main problem
        with time_code(solve_data.timing, 'mip'):
            mip_result = solve_LOA_main(solve_data, config)
...
def algorithm_should_terminate(solve_data, config):
    ...
    if solve_data.main_iteration >= config.iterlim:
        config.logger.info(
            'GDPopt unable to converge bounds '
            'after %s main iterations.'
            % (solve_data.main_iteration,))
        config.logger.info(
