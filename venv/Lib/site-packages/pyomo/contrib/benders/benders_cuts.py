...
Main problem must be of the form
...
def _setup_subproblem(b, main_vars, relax_subproblem_cons):
    # first get the objective and turn it into a constraint
    main_vars = ComponentSet(main_vars)
    ...
    for c in list(b.component_data_objects(pe.Constraint, descend_into=True, active=True, sort=True)):
        if not relax_subproblem_cons:
            ...
            if not _any_common_elements(main_vars, c_vars):
                continue
        if c.equality:
            body = c.body
...
class BendersCutGeneratorData(_BlockData):
    def __init__(self, component):
        ...
        self.main_vars = list()
        self.main_vars_indices = pe.ComponentMap()
        self.main_etas = list()
        self.cuts = None
        self.subproblem_solvers = list()
        self.tol = None
        self.all_main_etas = list()

    def set_input(self, main_vars, tol=1e-6):
        """
        It is very important for main_vars to be in the same order for every process.
        ...
        self.main_etas = list()
        self.complicating_vars_maps = list()
        self.main_vars = list(main_vars)
        self.main_vars_indices = pe.ComponentMap()
        for i, v in enumerate(self.main_vars):
            self.main_vars_indices[v] = i
        self.tol = tol
        self.subproblem_solvers = list()
        self.all_main_etas = list()

    def add_subproblem(self, subproblem_fn, subproblem_fn_kwargs, main_eta, subproblem_solver='gurobi_persistent', relax_subproblem_cons=False):
        _rank = np.argmin(self.num_subproblems_by_rank)
        self.num_subproblems_by_rank[_rank] += 1
        self.all_main_etas.append(main_eta)
        if _rank == MPI.COMM_WORLD.Get_rank():
            self.main_etas.append(main_eta)
            ...
            _setup_subproblem(subproblem, main_vars=[complicating_vars_map[i] for i in self.main_vars if i in complicating_vars_map], relax_subproblem_cons=relax_subproblem_cons)
    ...
    def generate_cut(self):
        coefficients = np.zeros(len(self.subproblems)*len(self.main_vars), dtype='d')
        ...
        for subproblem_ndx in range(len(self.subproblems)):
            ...
            main_eta = self.main_etas[subproblem_ndx]

            subproblem.fix_complicating_vars = pe.ConstraintList()
            var_to_con_map = pe.ComponentMap()
            for main_var in self.main_vars:
                if main_var in complicating_vars_map:
                    sub_var = complicating_vars_map[main_var]
                    sub_var.value = main_var.value
                    new_con = subproblem.fix_complicating_vars.add(sub_var - main_var.value == 0)
                    var_to_con_map[main_var] = new_con
            subproblem.fix_eta = pe.Constraint(expr=subproblem._eta - main_eta.value == 0)
            subproblem._eta.value = main_eta.value
        ...
        global_coeffs = np.zeros(total_num_subproblems*len(self.main_vars), dtype='d')
        ...
        for subproblem_ndx in range(total_num_subproblems):
            ...
            if cut_expr > self.tol:
                main_eta = self.all_main_etas[subproblem_ndx]
                cut_expr -= global_eta_coeffs[subproblem_ndx] * (main_eta - main_eta.value)
                for main_var in self.main_vars:
                    coeff = global_coeffs[coeff_ndx]
                    cut_expr -= coeff * (main_var - main_var.value)
                    coeff_ndx += 1
                new_cut = self.cuts.add(cut_expr <= 0)
                cuts_added.append(new_cut)
            else:
                coeff_ndx += len(self.main_vars)
        return cuts_added
