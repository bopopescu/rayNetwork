...
class TestBenders(unittest.TestCase):
    ...
    def test_grothey(self):
        ...
        def create_subproblem(main):
            ...
            complicating_vars_map[main.y] = m.y
        ...
        m.benders.set_input(main_vars=main_vars, tol=1e-8)
        m.benders.add_subproblem(subproblem_fn=create_subproblem,
                                 subproblem_fn_kwargs={'main': m},
                                 main_eta=m.eta,
    ...
    def test_farmer(self):
        ...
        def create_subproblem(main, farmer, scenario):
            m = pe.ConcreteModel()
            ...
            complicating_vars_map = pe.ComponentMap()
            for crop in m.crops:
                complicating_vars_map[main.devoted_acreage[crop]] = m.devoted_acreage[crop]

            return m, complicating_vars_map

        farmer = Farmer()
        m = create_main(farmer=farmer)
        main_vars = list(m.devoted_acreage.values())
        m.benders = BendersCutGenerator()
        m.benders.set_input(main_vars=main_vars, tol=1e-8)
