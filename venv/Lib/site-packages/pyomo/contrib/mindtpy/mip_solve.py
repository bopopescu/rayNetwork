...
def solve_OA_main(solve_data, config):
    solve_data.mip_iter += 1
    m = solve_data.mip.clone()
    MindtPy = m.MindtPy_utils
    config.logger.info(
        'MIP %s: Solve main problem.' %
        (solve_data.mip_iter,))
    # Set up MILP
    ...
    # m.pprint() #print oa main problem for debugging
    with SuppressInfeasibleWarning():
        results = SolverFactory(config.mip_solver).solve(
            m, **config.mip_solver_args)
    main_terminate_cond = results.solver.termination_condition
    if main_terminate_cond is tc.infeasibleOrUnbounded:
        # Linear solvers will sometimes tell me that it's infeasible or
        # unbounded during presolve, but fails to distinguish. We need to
        # resolve with a solver option flag on.
        results, main_terminate_cond = distinguish_mip_infeasible_or_unbounded(
            m, config)

    # Process main problem result
    if main_terminate_cond is tc.optimal:
    ...
    elif main_terminate_cond is tc.infeasible:
        config.logger.info(
        ...
        if solve_data.mip_iter == 1:
            config.logger.info(
                ...
                'quality cuts.')
    elif main_terminate_cond is tc.maxTimeLimit:
        ...
        config.logger.info(
            ...
            % (solve_data.mip_iter, value(MindtPy.obj.expr),
               solve_data.LB, solve_data.UB))
    elif (main_terminate_cond is tc.other and
        ...
        if main_objective.sense == minimize:
            ...
            solve_data.UB_progress.append(solve_data.UB)
    elif main_terminate_cond is tc.unbounded:
        ...
        raise ValueError(
            ...
            (main_terminate_cond, results.solver.message))

    # Call the MILP post-solve callback
    config.call_after_main_solve(m, solve_data)
